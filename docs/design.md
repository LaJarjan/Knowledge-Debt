...
| --- | --- | --- | --- |
| Concurrency | Function-local call whose final name is Semaphore or BoundedSemaphore | Callable spelling, initial argument | Object identity, protected operations, shared lifetime, downstream capacity |
| Retry | for-loop containing an exception handler and sleep call | Iterator, caught types, delay expression | Actual number of attempts, idempotency, complete control flow |
...
These are intentionally syntax candidates. Aliased names, custom callables,
while-loop retries, context-manager cleanup and more complex patterns may be
missed. Nested functions are not attributed to the containing function. No
classifier probability is exposed.

## Metrics

- **Debt:** prioritized checks within explicit responsibility scope.
- **Coverage:** current self-check count / supported instances in the selected
